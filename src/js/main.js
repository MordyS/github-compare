const brunches = ['master', 'chaim', 'mordy']

function compareOptions(_brunches) {
  return _brunches.reduce((acc, cur) => {
    _brunches.forEach(brunch => {
      if (brunch !== cur) acc.push([cur, brunch])
    })
    return acc
  }, [])
}
...
function addSelectMenue() {
  ...
  if (!menuElement || menuElement.querySelector('#compare-brunches')) {
    return;
  }
console.log(location)
  ...
  <div>
    <ul class="dropdown-menu dropdown-menu-sw" style="width: unset;">
      ${compareOptions(brunches).map(item =>
      `<li data-toggle-for="compare-brunches"><a href="${location.href}/compare/${item[0]}...${item[1]}" class="dropdown-item" target="_blank" rel="noopener noreferrer">
  ...
  detailsElement.setAttribute('id', 'compare-brunches');
